...
const EditPricingModal = ({ isOpen, onClose, onSubmit, pricingPackage, loading }) => {
  const [form, setForm] = useState({ name: "", description: "", discount_type: 0, discount_value: "" });
  useEffect(() => { if (pricingPackage) setForm({ name: pricingPackage.name || "", description: pricingPackage.description || "", discount_type: pricingPackage.discount_type || 0, discount_value: pricingPackage.discount_value || "" }); }, [pricingPackage]);
  if (!isOpen || !pricingPackage) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-[#1F1F20] border border-[#333] rounded-xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#333]">
          <h2 className="text-[#E5E5E5] font-bold text-lg">Edit Pricing Package</h2>
        ...
        <div className="px-6 py-4 border-t border-[#333] flex justify-end gap-3">
          ...
          <button onClick={() => onSubmit(form, pricingPackage.id)} disabled={loading} className="px-5 py-2 bg-[#FDCE06] text-black rounded-md text-sm font-bold disabled:opacity-50">{loading ? "Saving..." : "Save"}</button>
        </div>
      </div>
    </div>
  );
