...
interface EditContentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (contentData: any, contentId?: number) => Promise<void>;
  content: any;
  loading?: boolean;
}
...
const EditContentModal: React.FC<EditContentModalProps> = ({
  ...
  const [form, setForm] = useState({ equipment_id: "", equipment_name: "", description: "" });
  ...
  useEffect(() => {
    if (content) {
      ...
      setImages(Array.isArray(content.images) ? content.images : []);
      setErrors({});
    }
  }, [content]);
  ...
  const setMainImage = (index: number) => {
    setImages((prev) => prev.map((img, i) => ({ ...img, is_main: i === index ? 1 : 0 })));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      await onSubmit({ ...form, images }, content?.id);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-[#1F1F20] border border-[#333333] rounded-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#333333]">
          <h2 className="text-[#E5E5E5] font-[Inter] font-bold text-lg">Edit Content</h2>
        ...
        <div className="px-6 py-4 border-t border-[#333333] flex justify-end gap-3">
          <button onClick={handleClose} className="px-4 py-2 border border-[#444444] text-[#9CA3AF] rounded-md text-sm font-[Inter] hover:text-[#E5E5E5] transition-colors">
            Cancel
          </button>
          <button
