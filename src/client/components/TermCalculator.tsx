...
const TermCalculator = ({ equipment = [] }) => {
  // only machines that actually earn a compounding rate belong here
  const items = useMemo(
    () =>
      (equipment || []).filter(
  ...
  const calc = useMemo(() => {
    ...
    const flat = opening * months;
    const last = schedule[schedule.length - 1] || opening;
    return {
      schedule, total, flat, opening, last,
      saved: flat - total,
  ...
  if (!plant || !calc) return null;

  return (
    <section className="mb-12 lg:mb-16">
      <div className="bg-gradient-to-b from-[#212122] to-[#1B1B1C] border border-[#333333] rounded-2xl overflow-hidden">
        <div className="px-6 sm:px-9 pt-8 pb-7">

          <h2 className="text-[#E5E5E5] font-semibold text-[26px] sm:text-[32px] leading-[1.15] tracking-[-0.01em] mb-2">
            The longer you keep it,
            <span className="text-[#FDCE06]"> the less you pay.</span>
          </h2>
          <p className="text-[#9CA3AF] text-[14px] sm:text-[15px] leading-relaxed mb-8 max-w-xl">
            Your rate falls {calc.comp}{calc.cType === "%" ? "%" : ""} every month a machine stays on
            site. Move the slider and see what that's worth.
          </p>

          {/* controls */}
          <div className="grid lg:grid-cols-[1.15fr_1fr] gap-6 lg:gap-9 mb-9">
            <div>
              <label className="block text-[#9CA3AF] text-[12px] uppercase tracking-[0.07em] mb-2.5">
                Machine
              </label>
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none bg-[#292A2B] border border-[#3A3A3C] rounded-xl pl-5 pr-12 py-4 text-[#E5E5E5] text-[17px] font-medium outline-none focus:border-[#FDCE06] hover:border-[#4A4A4C] transition-colors cursor-pointer"
                >
                  {items.map((e) => (
            ...
            <div>
              <div className="flex items-baseline justify-between mb-2.5">
                <label className="text-[#9CA3AF] text-[12px] uppercase tracking-[0.07em]">
                  Term
                </label>
                <span className="text-[#E5E5E5] text-[17px] font-semibold tabular-nums">
                  {months} months
                </span>
              </div>
              <input
          ...
          {/* headline saving */}
          <div className="flex flex-wrap items-end gap-x-10 gap-y-6 mb-9">
            <div>
              <div className="text-[#9CA3AF] text-[12px] uppercase tracking-[0.07em] mb-1.5">
                You keep
              </div>
              <div className="text-[#FDCE06] font-semibold text-[42px] sm:text-[52px] leading-none tabular-nums transition-all duration-300">
                {money(calc.saved)}
              </div>
            </div>
            <div className="flex gap-8 pb-1.5">
              <div>
                ...
                <div className="text-[#9CA3AF] text-[19px] font-medium tabular-nums line-through decoration-[#4A4A4C]">
                  {money(calc.flat)}
                </div>
              </div>
              <div>
                <div className="text-[#6B7280] text-[12px] mb-1">Your total</div>
                <div className="text-[#4CAF50] text-[19px] font-semibold tabular-nums">
                  {money(calc.total)}
                </div>
              </div>
            </div>
          ...
          <div>
            <div className="flex items-baseline justify-between mb-3">
              <span className="text-[#9CA3AF] text-[12px] uppercase tracking-[0.07em]">
                Monthly rate
              </span>
              <span className="text-[#6B7280] text-[12px] tabular-nums">
                {money(calc.opening)} down to {money(calc.last)} · {calc.pct}% off
              </span>
            </div>
            <div className="flex items-end gap-[4px] h-[104px]">
              {calc.schedule.map((r, i) => (
                <div key={i} className="flex-1 group relative flex items-end h-full">
                  ...
                  />
                  <div className="pointer-events-none absolute -top-7 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-[#0F0F10] border border-[#3A3A3C] rounded px-2 py-1 text-[11px] text-[#E5E5E5] whitespace-nowrap tabular-nums">
                    {money(r)}
                  </div>
                </div>
              ))}
