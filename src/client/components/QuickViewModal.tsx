...
const QuickViewModal: React.FC<QuickViewModalProps> = ({
    ...
    const getMaintenanceInfo = () => {
        if (equipment?.maintenance_periods && Array.isArray(equipment.maintenance_periods) && equipment.maintenance_periods.length > 0) {
            ...
            const maintenance = equipment.maintenance_periods.find((p: any) => new Date(p.end_date) >= today);

            if (maintenance) {
                const startDate = new Date(maintenance.start_date);
                const endDate = new Date(maintenance.end_date);
                return {
                    start: startDate.toLocaleDateString('en-US', { day: '2-digit', month: 'long', year: 'numeric' }),
                    end: endDate.toLocaleDateString('en-US', { day: '2-digit', month: 'long', year: 'numeric' })
                };
            }
        }
        return null;
    ...
    // Calculate price based on duration and discounts (matching EquipmentCard logic)
    let totalCost = 0;
    let totalSavings = 0;
    ...
    const discountedPrice = initialPrice;
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="relative bg-[#2A2A2B] rounded-lg overflow-hidden aspect-[4/3] border-2 border-[#333333]">
                                ...
                                {/* Main Image */}
                                <img
                                    src={images[currentImageIndex]?.image_url || equipment.image_url || equipment.image}
                            ...
                            {equipment.unavailability_due_month && (
                                <div className="text-[#E5E5E5] text-sm font-semibold">
                                    Available {new Date(equipment.unavailability_due_month).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
                                </div>
                            )}
                            ...
                            <div className="space-y-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className={`text-lg font-bold ${hasDiscount ? 'line-through text-[#6B7280]' : 'text-[#E5E5E5]'}`}>
                                ...
                                {hasDiscount && (
                                    <div className="flex items-center gap-2">
                                        <span className="text-[#9CA3AF] text-sm">Discount:</span>
                        ...
                        <div className="space-y-4">
                            ...
                            {equipment?.maintenance_periods && equipment.maintenance_periods.length > 1 && (
                                <div className="space-y-1 text-xs">
                                    {equipment.maintenance_periods.slice(1).map((period: any, index: number) => {
                                        const start = new Date(period.start_date).toLocaleDateString('en-US', { day: '2-digit', month: 'long', year: 'numeric' });
                                        const end = new Date(period.end_date).toLocaleDateString('en-US', { day: '2-digit', month: 'long', year: 'numeric' });
                                        return (
